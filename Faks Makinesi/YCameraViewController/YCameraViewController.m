...
- (void)viewDidLoad
{
    ...
    [[CommonHelper appDelegate] setTabbarHidden:YES];

    self.lblFlash.text =NSLocalizedString(@"TXT_FLASH_OFF", nil);
    [self.flashToggleButton setImage:[UIImage imageNamed:@"ic_on.png"] forState:UIControlStateNormal];
    ...
    [self.libraryToggleButton setTitle:NSLocalizedString(@"TXT_GALLERY", nil) forState:UIControlStateNormal];
    
    // Initialize Motion Manager
    [self initializeMotionManager];
    [self.lblFlash setFont:[UIFont fontWithName:FONT_NAME size:17]];
}
...
//            captureVideoPreviewLayer.frame = CGRectMake(0,65,320,421);
//        }
//        else
//        {
//            captureVideoPreviewLayer.frame = CGRectMake(0,65,320,421-88);
//        }
//    }

        if (IS_IPHONE_4) {
             self.imagePreview.frame = CGRectMake(0,0,ScreenWidth,ScreenHeight-64-82);
        }
        else
        {
             self.imagePreview.frame = CGRectMake(0,64,ScreenWidth,ScreenHeight-64-82);
        }

    NSLog(@"%f---%f---%f---%f",self.imagePreview.bounds.origin.x,self.imagePreview.bounds.origin.y,self.imagePreview.bounds.size.width,self.imagePreview.bounds.size.height);
            ...
            if (self.flashToggleButton.selected)
                {
                    ...
                    self.lblFlash.text =NSLocalizedString(@"TXT_FLASH_ON", nil);
                }
...
//    UIImage *smallImage = [self imageWithImage:image scaledToWidth:320.0f]; //UIGraphicsGetImageFromCurrentImageContext();
//    
...
- (void)setCapturedImage{
    ...
    pictureVC.imgCapture = self.captureImage.image;
